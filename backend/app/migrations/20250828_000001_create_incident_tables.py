...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("remediation_candidates", pg.JSONB, nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("validation_results", pg.JSONB, nullable=False, server_default=sa.text("'[]'::jsonb")),
    )
    op.create_index("ix_incidents_service_created", "incidents", ["service", "created_at"], unique=False)
